...
export default function AdminSettingsPage() {
    ...
    const fetchSession = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (data.authenticated && (data.user.role === "SUPERADMIN" || data.user.role === "TEACHER")) {
            setUser(data.user);
  ...
  return (
    <div className="max-w-2xl mx-auto w-full space-y-6">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-display font-black text-slate-800 tracking-tight">Pengaturan Admin</h1>
        <p className="text-slate-500 font-sans font-semibold text-xs mt-1">
          Ubah nama profil admin atau perbarui kata sandi keamanan Anda.
        </p>
      </div>
