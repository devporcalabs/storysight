      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-12">
        ...
        {loading ? (
          ...
          <>
            ...
            <section className="relative w-full h-[320px] sm:h-[380px] md:h-[480px] rounded-[24px] sm:rounded-[32px] overflow-hidden shadow-xl bg-slate-900 group">
              {/* Carousel Slides */}
              {featuredBanners.map((banner, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Content */}
                    <div className="absolute bottom-6 left-4 right-4 sm:bottom-8 sm:left-6 md:bottom-16 md:left-16 max-w-xl text-white z-20 space-y-3 sm:space-y-4">
                      {/* Tags */}
                      ...
                      {/* Title */}
                      <h1 className="font-display text-2xl sm:text-3xl md:text-5xl font-black tracking-tight leading-tight sm:leading-none drop-shadow-md">
                        {story.title}
                      </h1>

                      {/* Description */}
                      <p className="text-slate-300 text-[10px] sm:text-xs md:text-sm font-semibold line-clamp-2 sm:line-clamp-3 leading-relaxed drop-shadow">
                        {story.description}
                      </p>
                      ...
                      <div className="flex flex-wrap gap-2 sm:gap-3 pt-1">
                        <Link
                          ...
                          className="inline-flex items-center justify-center gap-1.5 bg-white text-slate-950 font-display text-[10px] sm:text-xs md:text-sm font-black px-4 py-2.5 sm:px-6 sm:py-3 rounded-full hover:bg-slate-200 transition-all shadow-lg active:scale-95"
                        >
                          <Play className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-current" /> Putar sekarang
                        </Link>
                        <button
                          onClick={() => toggleFavorite(story.id)}
                          className="inline-flex items-center justify-center gap-1.5 bg-white/20 backdrop-blur-md text-white font-display text-[10px] sm:text-xs md:text-sm font-black px-4 py-2.5 sm:px-6 sm:py-3 rounded-full border border-white/25 hover:bg-white/30 transition-all shadow-lg active:scale-95 cursor-pointer"
                        >
                          {favorites.includes(story.id) ? (
                            <>
                              <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-400" /> Tersimpan
                            </>
                          ) : (
                            <>
                              <Plus className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Favorit Saya
                            </>
                          )}
                        </button>
                      </div>
              ...
              {/* Slider Dots */}
              <div className="absolute bottom-4 right-4 sm:bottom-6 sm:right-6 md:right-16 flex gap-1.5 z-35">
            ...
            <section className="space-y-4">
              ...
              <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-none snap-x snap-mandatory">
                {trendingStories.map((story) => (
                  ...
                  >
                    {/* Poster Card */}
                    <div className="relative aspect-video w-full rounded-2xl overflow-hidden glass-card shadow-sm border-white/20 transition-all duration-500 group-hover:scale-[1.03] group-hover:shadow-md group-hover:border-primary/20">
                      <img
              ...
              {filteredCategoryStories.length === 0 ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  {filteredCategoryStories.map((story) => (
                    ...
                    >
                      <div className="relative aspect-video w-full rounded-xl overflow-hidden glass-card shadow-sm border-white/20 transition-transform duration-300 group-hover:scale-[1.02]">
                        <img
                          src={story.thumbnailUrl}
                          alt={story.title}
                          className="w-full h-full object-cover"
            ...
            <section className="glass-card p-6 md:p-8 rounded-[32px] border-white/20 space-y-6">
              ...
              {leaderboard.length === 0 ? (
                ...
                <div className="overflow-hidden border border-slate-200/40 rounded-2xl bg-white/30 backdrop-blur-md">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="border-b border-slate-200/30 text-slate-400 font-extrabold uppercase tracking-wider font-mono bg-slate-900/5">
                          <th className="py-3 px-4 w-16 text-center">Rank</th>
                          <th className="py-3 px-4">Nama Siswa</th>
                          <th className="py-3 px-4">Asal Sekolah</th>
                          <th className="py-3 px-4 text-center">Total Kuis</th>
                      ...
                      <tbody className="divide-y divide-slate-100/50 font-semibold text-slate-700">
                        {leaderboard.map((student, index) => {
                          ...
                          let rankBadge = null;
                          if (rank === 1) {
                            rankBadge = <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-amber-100 text-amber-600 border border-amber-200 shadow-sm" title="Juara 1">👑</span>;
                          } else if (rank === 2) {
                            rankBadge = <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-slate-200 text-slate-600 border border-slate-300 shadow-sm font-bold text-[10px]" title="Juara 2">2</span>;
                          } else if (rank === 3) {
                            rankBadge = <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-amber-50 text-amber-700 border border-amber-200 shadow-sm font-bold text-[10px]" title="Juara 3">3</span>;
                          } else {
                            rankBadge = <span className="text-slate-400 font-mono font-bold text-[10px]">{rank}</span>;
                          }

                          return (
                            <tr key={student.id} className="hover:bg-primary/5 transition-all duration-150">
